...
class GetPostPDF : AddReactionDelegate,SharePostDelegate,comment_CountsDelegate{
    ...
    func getPostPDF (targetControler: UIViewController,tableView : UITableView, indexpath:IndexPath, postfile : String ,array : [[String:Any]], stackViewHeight: CGFloat,viewHeight: CGFloat, isHidden: Bool, viewColor :UIColor) -> UITableViewCell {
        self.targetController = targetControler
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: "PostPDFCell") as! PostPDFCell
    ...
    @IBAction func NormalTapped(gesture: UIGestureRecognizer){
        ...
        case .online(.wwan), .online(.wiFi):
            guard let cell = self.tableView.cellForRow(at: IndexPath(row: 0, section: gesture.view!.tag + sumAmount)) as? PostPDFCell else { return }
    ...
    func comment_Count() {
        guard let cell = self.tableView.cellForRow(at: IndexPath(row: 0, section: self.selectedIndex+sumAmount)) as? PostPDFCell else { return }
...
//                            BoostPostManager.sharedInstance.boostPosts(targetController: self.targetController, postId: post_id ?? "")
    ...
    static let sharedInstance = GetPostPDF()
    private init() {
