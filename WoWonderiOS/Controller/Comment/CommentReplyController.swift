...
class CommentReplyController: UIViewController,UITextViewDelegate,uploadImageDelegate,AddReactionDelegate,AVAudioRecorderDelegate,AVAudioPlayerDelegate{
            ...
            if let reactions = self.commentReplies[gesture.view?.tag ?? 0]["reaction"] as? [String:Any]{
                ...
                if let isReacted = reactions["is_reacted"] as? Bool{
                    if (isReacted == true){
                        ...
                        var localPostArray = (self.commentReplies[gesture.view?.tag ?? 0]["reaction"] as? [String:Any]) ?? [String:Any]()
    ...
    func addReaction(reation: String) {
        ...
        var localPostArray = (self.commentReplies[self.selectedIndex]["reaction"] as? [String:Any]) ?? [String:Any]()
        var totalCount = 0
...
//        cell.audioTimer.text = "00:\(counter)"
    ...
    @objc func playingAudio(sender:UIButton){
        ...
        let record = self.comment["record"] as? String ?? ""
...
extension CommentReplyController : UITableViewDelegate,UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.section == 0 {
            ...
            if let audioFile = self.comment["record"] as? String{
                if audioFile != ""{
