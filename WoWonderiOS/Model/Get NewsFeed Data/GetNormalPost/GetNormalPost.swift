...
class GetNormalPost :AddReactionDelegate,SharePostDelegate,comment_CountsDelegate {
    ...
    @objc func seeMoreBtnClicked(_ sender: UIButton) {
        print("sinpanda")
        if flag {
    ...
    func getPostText (targetController: UIViewController,tableView : UITableView, indexpath:IndexPath, postFile : String, array : [[String:Any]],stackViewHeight: CGFloat,viewHeight: CGFloat, isHidden: Bool, viewColor :UIColor) -> UITableViewCell{
...
//        cell.ShareBtn.isHidden = isHidden
     ...
     //   cell.profileName.addGestureRecognizer(gestureonLabel)
        ...
        cell.profileButton.addTarget(self, action: #selector(self.gotoProfile), for: .touchUpInside)
...
//            cell.statusLabel.text! = getHeading(tempText: textStatus.htmlToString)
            if textStatus.count > 100 {
                if flag {
                    cell.statusLabel.text = getHeading(tempText: textStatus) + "..."
                }
                else {
                    cell.statusLabel.text = textStatus
                }
                
                cell.seeMoreBtn.addTarget(self, action: #selector(seeMoreBtnClicked), for: .touchUpInside)
            }
            else {
                cell.statusLabel.text = textStatus
        ...
        if let postMaps = index["postMap"] as? String{
            if postMaps != "" {
                PostMap = postMaps
                print(postMaps)
                cell.widthContraint.constant = 0
                cell.locationLabel.text = "\("postMap")\(" ")\(postMaps)"
                cell.iconImage?.image = #imageLiteral(resourceName: "maps-and-flags")
        ...
        if let postWatching = index["postWatching"] as? String{
            if postWatching != "" {
                cell.widthContraint.constant = 0
                cell.locationLabel.text = "\("is Watching")\(" ")\(postWatching)"
                cell.iconImage?.image = #imageLiteral(resourceName: "view")
...
//        for i in self.commentsIndex{
//            if i["index"] as? Int == indexpath.row{
//                cell.sharesCountBtn.setTitle("\(" ")\(i["count"] ?? "")", for: .normal)
//            }
//        }
    ...
    @IBAction func NormalTapped(gesture: UIGestureRecognizer){
        ...
        case .online(.wwan), .online(.wiFi):
            guard let tag = gesture.view?.tag, tag < self.postArray.count else { return }
            guard let cell = self.tableView.cellForRow(at: IndexPath(row: 0, section: tag + sumAmount)) as? NormalPostCell else { return }
    ...
    @objc func gotoProfile(){
        ...
        let controller = (story.instantiateViewController(identifier: "UserProfileVC") as? UserProfileVC)!
        controller.user_id = self.postArray[self.selectedIndex]["user_id"] as? String
        targetController.navigationController?.pushViewController(controller, animated: true)
    }
...
//                            BoostPostManager.sharedInstance.boostPosts(targetController: self.targetController, postId: post_id ?? "")
    ...
    static let sharedInstance = GetNormalPost()
    private init() {
