...
class GroupController: UIViewController,GroupMoreDelegate,editPostDelegate{
...
//    }
    ...
    func editPost(newtext: String, postPrivacy: String) {
        self.groupPostsArray[self.selectedIndex]["postText"] = newtext
        self.groupPostsArray[self.selectedIndex]["postPrivacy"] = postPrivacy
        self.tableView.reloadData()
    }
...
private func uploadImage(imageType:String, data:Data){
    ...
    if let groupId = self.groupData["group_id"] as? String{
        group_id = groupId
    }
     ...
     self.tableView.rowHeight = 362.0
        ...
        if let user_id = self.groupData["user_id"] as? String{
            ...
            if user_id == UserData.getUSER_ID(){
                ...
                cell.joinGroupBtn.backgroundColor = UIColor.hexStringToUIColor(hex: ControlSettings.buttonColor)
