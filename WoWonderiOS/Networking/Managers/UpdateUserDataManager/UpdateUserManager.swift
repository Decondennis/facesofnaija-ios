...
class UpdateUserManager{
    ...
    func updateProfile(firstName: String,lastName:String,mobile:String,website:String,workSpace:String,school:String,location:String,relation:String,completionBlock :@escaping (_ Success: UpdateUserModel.UpdateUserDataSuccessModel?, _ AuthError: UpdateUserModel.UpdateUserDataErrorModel?, Error?)->()){
        let params = [
            APIClient.Params.serverKey:APIClient.SERVER_KEY.Server_Key,
     ...
     func deleteAccount(currentPassword: String,completionBlock :@escaping (_ Success: UpdateUserModel.UpdateUserDataSuccessModel?, _ AuthError: UpdateUserModel.UpdateUserDataErrorModel?, Error?)->()){
            ...
            let access_token = "\("?")\("access_token")\("=")\(UserData.getAccess_Token()!)"
        AF.request(APIClient.DeleteUser.deleteUserApi + access_token, method: .post, parameters: params, encoding: URLEncoding.default, headers: nil).responseJSON { (response) in
                if response.value != nil{
                    guard let res = response.value as? [String:Any] else {return}
                    guard let apiStatusCode = res["api_status"] as? Any else {return}
                    if apiStatusCode as? Int == 200{
