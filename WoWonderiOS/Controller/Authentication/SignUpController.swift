...
class SignUpController: BaseVC {
    ...
    @IBAction func Register(_ sender: Any) {
      if self.userNameField.text?.isEmpty == true {
        ...
        else if self.genderField.text?.isEmpty == true{
            self.error = NSLocalizedString("Error, Please select gender", comment: "Error, Please select gender")
            self.performSegue(withIdentifier: "ErrorVC", sender: self)
        }
    ...
    @IBAction func PrivacyPolicy(_ sender: Any) {
        if let url = URL(string: "\(APIClient.baseURl)/terms/privacy-policy") {
            UIApplication.shared.open(url)
        }
    }
