...
class GetPostWithImage: AddReactionDelegate,SharePostDelegate,comment_CountsDelegate{
...
//        cell.heigthConstraint.constant = 220.0
        ...
        cell.commentAction = {[unowned self] in
            ...
            if let status = index["comments_status"] as? String{
            ...
            //            }
            if let counts = index["post_comments"] as? String{
        ...
        cell.share_link = {[unowned self] in
            var postUrl = (index["url"] as? String) ?? ""
            if postUrl.isEmpty {
                let postId = (index["post_id"] as? String) ?? "\(index["post_id"] ?? "")"
        ...
        cell.moreAction = { [unowned self] in
            ...
            if let publisher = index["publisher"] as? [String:Any]{
                if let is_myPost = publisher["user_id"] as? String{
                    if is_myPost == UserData.getUSER_ID(){
                        if let enableComment = index["comments_status"] as? String{
        ...
        cell.normalAction = {[unowned self] in
            ...
            case .online(.wwan), .online(.wiFi):
                ...
                if is_react {
                    ...
                    if let idx = self.selectedIndexs.firstIndex(where: { ($0["index"] as? Int) == indexpath.row }) {
                        self.selectedIndexs[idx] = action
                    } else {
                        self.selectedIndexs.append(action)
                    }
        ...
        let attributedString10 = NSMutableAttributedString(string: "\(" ")\(postTypes)", attributes:attrs3)
    ...
    func sharePostLink() {
        ...
        } else if let shareData = self.sharePostData {
            postUrl = (shareData["url"] as? String) ?? ""
            if postUrl.isEmpty {
                let postId = (shareData["post_id"] as? String) ?? "\(shareData["post_id"] ?? "")"
                if !postId.isEmpty && postId != "0" {
                    postUrl = "\(APIClient.baseURl)/post/\(postId)"
                }
            }
