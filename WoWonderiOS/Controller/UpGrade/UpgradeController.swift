...
extension UpgradeController : UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout{
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.upgradeBtn.addTarget(self, action: #selector(self.openPaymentVC), for: .touchUpInside)
        }
    ...
    func startCheckout(){
        ...
        let request = BTPayPalRequest(amount:  (self.upgardeArray[ self.selectedIndex ?? 0]["price"] as? String)!)
...
extension UpgradeController: didSelectPaymentTypeDelegate{
...
extension UpgradeController: paystackDelegate{
        ...
        func sendEmail(email: String) {
            ...
            vc.email = email
            var amounts = 0
            if let price = self.upgardeArray[ self.selectedIndex ?? 0]["price"] as? String{
                amounts = Int(price) ?? 1
            }
            vc.amount = amounts
