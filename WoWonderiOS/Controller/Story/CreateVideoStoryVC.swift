...
class CreateVideoStoryVC: UIViewController {
    ...
    private func createStory1() {
        let text = self.captionTxtView.text ?? ""
        
        let url = URL(string: self.videoLinkString!)
        let videoData = try? Data(contentsOf: url!)
        
        StoriesManager.sharedInstance.createStory(story_data: videoData1, mimeType: videoData1!.mimeType, type: "video", text: text, completionBlock: { (success, sessionError, error) in
            if success != nil{
