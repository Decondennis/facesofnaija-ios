...
class GetPostEvent : AddReactionDelegate,SharePostDelegate,comment_CountsDelegate{
    ...
    func getEvent (targetController :UIViewController,tableView : UITableView, indexpath:IndexPath, postFile : String, array : [[String:Any]],stackViewHeight: CGFloat,viewHeight: CGFloat, isHidden: Bool, viewColor :UIColor) -> UITableViewCell {
        
        self.targetController = targetController
        self.tableView = tableView
        self.postArray = array
        let indexs = array[indexpath.row]
        ...
        if let event = index["event"] as? [String:Any] {
            if let name = event["name"] as? String {
                cell.eventName.text = name
                
            }
            if let desc = event["description"] as? String {
            ...
            if let startDate = event["start_date"] as? String {
                cell.startDate.text = startDate
            }
            if let image = event["cover"] as? String {
                var cellFrame = cell.frame.size
                cellFrame.height =  cellFrame.height - 15
                cellFrame.width =  cellFrame.width - 15
                let url = URL(string:image)
                cell.eventImage.sd_setImage(with: url, placeholderImage: #imageLiteral(resourceName: "d-cover"), options: [], progress: nil, completed: nil)
            }
        }
        ...
        cell.btnAction = {[unowned self] in
            ...
            if let event = indexs["event"] as? [String:Any]{
                vc.eventDetail = event
            }           
            self.targetController?.navigationController?.pushViewController(vc, animated: true)
        }
        ...
        imageAttachment2.image = UIImage(named: "rightGray")
        ...
        let attributedString8 = NSMutableAttributedString(string: "\("  ")\("Created new Event")", attributes:attrs3)
    ...
    @IBAction func NormalTapped(gesture: UIGestureRecognizer){
        ...
        case .online(.wwan), .online(.wiFi):
            guard let cell = self.tableView.cellForRow(at: IndexPath(row: 0, section: gesture.view!.tag + sumAmount)) as? EventCell else { return }
    ...
    func addReaction(reation: String) {
        let cell = self.tableView.cellForRow(at: IndexPath(row: 0, section: self.selectedIndex+sumAmount)) as? EventCell
    ...
    func comment_Count() {
        guard let cell = self.tableView.cellForRow(at: IndexPath(row: 0, section: self.selectedIndex+sumAmount)) as? EventCell else { return }
...
//                            BoostPostManager.sharedInstance.boostPosts(targetController: self.targetController, postId: post_id ?? "")
    ...
    static let sharedInstance = GetPostEvent()
    private init () {
