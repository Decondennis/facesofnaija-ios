<?php
// Write debug info to a file instead of error_log
...
if (!empty($_POST["user_id"])) {
    ...
    function my_save_file($tmp, $name, $type, $uid) {
        global $debug_file;
        file_put_contents($debug_file, "my_save_file: tmp=$tmp, name=$name, type=$type, uid=$uid\n", FILE_APPEND);
        ...
        $dest = "$dir/$filename";
        $relative_path = "upload/photos/$month/$filename";
        
        file_put_contents($debug_file, "Saving to: $dest\n", FILE_APPEND);
        
        if (move_uploaded_file($tmp, $dest)) {
            file_put_contents($debug_file, "SUCCESS: File saved\n", FILE_APPEND);
            return $relative_path;
        } else {
            file_put_contents($debug_file, "FAILED: move_uploaded_file failed, error: " . error_get_last()['message'] . "\n", FILE_APPEND);
            if (copy($tmp, $dest)) {
                file_put_contents($debug_file, "SUCCESS: File copied\n", FILE_APPEND);
                return $relative_path;
            } else {
                file_put_contents($debug_file, "FAILED: copy also failed\n", FILE_APPEND);
            }
        }
        return "";
    }
    ...
    if (!empty($photos["tmp_name"])) {
        file_put_contents($debug_file, "Processing photos\n", FILE_APPEND);
