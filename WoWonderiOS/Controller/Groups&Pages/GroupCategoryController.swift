...
class GroupCategoryController: UIViewController {
    ...
    var groupCategories = [String:String]()
    ...
    override func viewDidLoad() {
        ...
        if self.isFromPage == 1{
            if let productCat = config["page_categories"] as? [String:String]{
                let unsortedDic = productCat
                self.groupCategories = unsortedDic
    //                unsortedDic.keys.sorted(by: <)
                self.tableView.reloadData()
            }
        }
        else{
            if let productCat = config["group_categories"] as? [String:String]{
                let unsortedDic = productCat
                self.groupCategories = unsortedDic
    //                unsortedDic.keys.sorted(by: <)
                self.tableView.reloadData()
...
extension GroupCategoryController: UITableViewDelegate,UITableViewDataSource{
    
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        self.groupCategories.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = UITableViewCell()
        let values = Array(self.groupCategories.values)[indexPath.row]
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let values = Array(self.groupCategories.values)[indexPath.row]
        let keys = Array(self.groupCategories.keys)[indexPath.row]
        print(keys,values)
        self.dismiss(animated: true) {
            self.delegate.selectCategory(categoryID: Int(keys) ?? 0, categoryName: NSLocalizedString(values, comment: values))
        }
    }
