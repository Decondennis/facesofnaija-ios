...
class GetPostVideo :AddReactionDelegate,SharePostDelegate,comment_CountsDelegate {
    ...
    func getVideo(targetController :UIViewController,tableView : UITableView, indexpath:IndexPath, postFile : String, array : [[String:Any]], url : URL,stackViewHeight: CGFloat,viewHeight: CGFloat, isHidden: Bool, viewColor :UIColor) -> UITableViewCell{
        ...
        cell.controls.skipForwardButton?.isHidden = true
        cell.controls.skipBackwardButton?.isHidden = true
    ...
    @IBAction func NormalTapped(gesture: UIGestureRecognizer){
        ...
        case .online(.wwan), .online(.wiFi):
            let cell = self.tableView.cellForRow(at: IndexPath(row: 0, section: gesture.view!.tag + sumAmount)) as! VideoCell
...
//        }
    ...
    @objc func playVideo (_ sender : UIButton){
        ...
        let cell = tableView.cellForRow(at: IndexPath(row: 0, section: indexPath.row + sumAmount)) as! VideoCell
        cell.videoView.play()
...
//                            BoostPostManager.sharedInstance.boostPosts(targetController: self.targetController, postId: post_id ?? "")
    ...
    static let sharedInstance = GetPostVideo()
    private init() {
