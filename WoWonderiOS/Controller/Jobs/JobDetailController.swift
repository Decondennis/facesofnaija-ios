...
class JobDetailController: UIViewController,EditJobDelegate,EditJobDataDelegate,DeleteJobDelegate{
    ...
    @IBAction func ShowApplies(_ sender: Any) {
        if self.appliesBtn.title(for: .normal) == NSLocalizedString("Apply NOW", comment: "Apply NOW"){
            print("ApplyNow")
            ...
            vc.jobId = "\(self.jobId)"
            vc.titleString = self.jobTitle.text
    ...
    private func loadData(){
        ...
        if let jobTitle = self.jobData["title"] as? String{
            self.jobTitle.text = jobTitle.htmlToString
        ...
        if let descrip = self.jobData["description"] as? String{
            self.jobDescription.text = descrip.htmlToString
        }
