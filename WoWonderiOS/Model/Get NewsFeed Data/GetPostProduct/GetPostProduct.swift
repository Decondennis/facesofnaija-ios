...
class GetPostProduct :AddReactionDelegate,SharePostDelegate,comment_CountsDelegate{
    ...
    func GetProduct (targetController :UIViewController,tableView : UITableView, indexpath:IndexPath, postFile : String, array : [[String:Any]],stackViewHeight: CGFloat,viewHeight: CGFloat, isHidden: Bool, viewColor :UIColor) -> UITableViewCell{
        ...
        if let product = index["product"] as? [String:Any]{
            if let location = product["location"] as? String {
            ...
            if let productDescription = product["description"] as? String{
                cell.productDescription.text = productDescription
            }
            
            if let productType = product["type"] as? String {
                if productType == "0"{
                    cell.typeLabel.text = "New"
                }
                else {
                    cell.typeLabel.text = "Use"
                }
                
            }
            
            if let status = product["status"] as? String{
                if status == "0" {
            ...
            if let images = product["images"] as? [[String:Any]] {
                for i in images {
                    let url = URL(string: i["image"] as! String)
//                    cell.productImage.kf.setImage(with: url, placeholder: nil, options: [], progressBlock: nil) { [weak self](image, error, cache, url) in
//                        if image != nil {
//                            cell.heightConstraint.constant = GetPosts.getAspectRatioAccordingToiPhones(cellImageFrame:cellFrame,downloadedImage: image!)
//                            
...
//                        }
//                        else {
//                            cell.heightConstraint.constant = 250.0
//                        }
//                    }
                }
            }
        ...
        cell.productDetailBtn.tag = indexpath.row
        ...
        cell.productDetailBtn.addTarget(self, action: #selector(self.gotoProductDetail(sender:)), for: .touchUpInside)
        ...
        let attributedString6 = NSMutableAttributedString(string: "\(" ")\("added new product for sell")", attributes:attrs3)
    ...
    @IBAction func NormalTapped(gesture: UIGestureRecognizer){
        ...
        case .online(.wwan), .online(.wiFi):
            guard let tag = gesture.view?.tag, tag < self.postArray.count else { return }
            guard let cell = self.tableView.cellForRow(at: IndexPath(row: 0 , section: tag + sumAmount)) as? ProductCell else { return }
    ...
    private func reactions(index :Int, reaction: String) {
        performUIUpdatesOnMain {
            guard index < self.postArray.count else { return }
    ...
    func addReaction(reation: String) {
        ...
        let cell = self.tableView.cellForRow(at: IndexPath(row: 0, section: self.selectedIndex+sumAmount)) as? ProductCell
    ...
    func comment_Count() {
        guard let cell = self.tableView.cellForRow(at: IndexPath(row: 0, section: self.selectedIndex+sumAmount)) as? ProductCell else { return }
    ...
    @IBAction func gotoProductDetail(sender: UIButton){
        if (AppInstance.instance.vc == "newsFeedVC"){
            let userInfo = ["userData":sender.tag,"type":"product"] as [String : Any]
            NotificationCenter.default.post(name: NSNotification.Name(rawValue: "performSegue"), object: nil, userInfo: userInfo)
        }
    }
...
//                            BoostPostManager.sharedInstance.boostPosts(targetController: self.targetController, postId: post_id ?? "")
    ...
    static let sharedInstance = GetPostProduct()
    private init() {
