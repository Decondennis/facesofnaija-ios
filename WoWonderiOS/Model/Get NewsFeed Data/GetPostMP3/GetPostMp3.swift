...
class GetPostMp3 :AddReactionDelegate,SharePostDelegate,comment_CountsDelegate{
    ...
    func getMP3 (targetController :UIViewController,tableView : UITableView, indexpath:IndexPath, postFile : String, array : [[String:Any]], stackViewHeight: CGFloat,viewHeight: CGFloat, isHidden: Bool, viewColor :UIColor) -> UITableViewCell {
        ...
        if let postMp3 = index["postFile"] as? String {
            let postURl = postMp3
            if postURl != self.url{
                self.url = postURl
...
//                            BoostPostManager.sharedInstance.boostPosts(targetController: self.targetController, postId: post_id ?? "")
    ...
    static let sharedInstance = GetPostMp3()
    private init() {
