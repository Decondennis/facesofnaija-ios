...
class MoviesCommentManager {
    ...
    func likeComment (MovieId:String,commentId: String, type: String,reaction:String, completionBlock : @escaping (_ Success:
          ...
          AF.request(APIClient.Movies_Comment.MovieCommentLikeDislike + access_token, method: .post, parameters: params, encoding: URLEncoding.default, headers: nil).responseJSON { (response) in
                 if response.value != nil {
                     guard let res = response.value as? [String:Any] else {return}
                     guard let apiStatusCode = res["api_status"] as? Any else {return}
    ...
    func addComment (MovieId:String, type: String, text:String,completionBlock : @escaping (_ Success:
        CreateCommentModel.createComment_SuccessModel?, _ AuthError : CreateCommentModel.createComment_ErrorModel? , Error?)->()){
            ...
            AF.request(APIClient.Movies_Comment.CreateMovieComment + access_token, method: .post, parameters: params, encoding: URLEncoding.default, headers: nil).responseJSON { (response) in
                   if response.value != nil {
                       guard let res = response.value as? [String:Any] else {return}
                       guard let apiStatusCode = res["api_status"] as? Any else {return}
                       if apiStatusCode as? Int == 200{
    ...
    func fetchCommentReply (commentId : String, offset : String,completionBlock : @escaping (_ Success:FetchCommentModel.fetchComment_SuccessModel?, _ AuthError : FetchCommentModel.fetchComment_ErrorModel? , Error?)->()){
        ...
        let access_token = "\("?")\("access_token")\("=")\(UserData.getAccess_Token()!)"
        AF.request(APIClient.Movies_Comment.getCommentReplies + access_token, method: .post, parameters: params, encoding: URLEncoding.default, headers: nil).responseJSON { (response) in
            if response.value != nil {
                print("Res = \(response.value)")
                guard let res = response.value as? [String:Any] else {return}
                guard let apiStatusCode = res["api_status"]  as? Any else {return}
    ...
    func commentReplyLikeDislike (MovieId:String,commentId: String, type: String,reaction:String, completionBlock : @escaping (_ Success:
             ...
             AF.request(APIClient.Movies_Comment.CommentReplyLikeDislikep + access_token, method: .post, parameters: params, encoding: URLEncoding.default, headers: nil).responseJSON { (response) in
                    if response.value != nil {
                        guard let res = response.value as? [String:Any] else {return}
                        guard let apiStatusCode = res["api_status"] as? Any else {return}
                        if apiStatusCode as? Int == 200{
    ...
    func addCommentReply(MovieId:String, commentID:String,type: String, text:String,completionBlock : @escaping (_ Success:
           CreateCommentModel.createComment_SuccessModel?, _ AuthError : CreateCommentModel.createComment_ErrorModel? , Error?)->()){
               ...
               AF.request(APIClient.Movies_Comment.addCommentReply + access_token, method: .post, parameters: params, encoding: URLEncoding.default, headers: nil).responseJSON { (response) in
                      if response.value != nil {
                          guard let res = response.value as? [String:Any] else {return}
                          guard let apiStatusCode = res["api_status"] as? Any else {return}
                          if apiStatusCode as? Int == 200{
