    ...
    private func loadMyFunding(offset: Int){
        GetUserFundingManager.sharedInstance.getFunding(userId: UserData.getUSER_ID() ?? "", offset: "\(offset)") { (success, authError, error) in
            if (success != nil){
                ...
                let off_set = self.myFunding.last?["id"] as? Int ?? 0
                self.offset = "\(off_set)"
                print(offset)
...
//        }
        ...
        if let last_time = index["time"] as? String{
...
//            self.timeLabel.text = "\(dat)"
        ...
        if let bar = index["bar"] as? Int{
            cell.progressBar.barColor = UIColor.hexStringToUIColor(hex: ControlSettings.appMainColor)
