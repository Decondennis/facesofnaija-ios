...
class GetPostBlog :AddReactionDelegate,SharePostDelegate,comment_CountsDelegate{
    ...
    func GetBlog (targetController :UIViewController,tableView : UITableView, indexpath:IndexPath, postFile : String, array : [[String:Any]],stackViewHeight: CGFloat,viewHeight: CGFloat, isHidden: Bool, viewColor :UIColor) -> UITableViewCell{
        ...
        if let textStatus = index["postText"] as? String {
            let text  = textStatus.htmlToString
        ...
        if let blog = index["blog"] as? [String:Any]{
            if let cover = blog["thumbnail"] as? String{
            ...
            if let category_name = blog["category_name"] as? String{
                cell.categoryLabel.text = category_name.htmlToString
            }
            if let description = blog["description"] as? String{
                cell.blogDescription.text = description.htmlToString
            }
        }
        ...
        let attributedString1 = NSMutableAttributedString(string: pro_name, attributes:attrs1)
        ...
        let attributedString6 = NSMutableAttributedString(string: "\(" ")\("created new article")", attributes:attrs3)
    ...
    @IBAction func NormalTapped(gesture: UIGestureRecognizer){
        ...
        case .online(.wwan), .online(.wiFi):
            guard let cell = self.tableView.cellForRow(at: IndexPath(row: 0, section: gesture.view!.tag + sumAmount)) as? BlogCell else { return }
    ...
    func addReaction(reation: String) {
        let cell = self.tableView.cellForRow(at: IndexPath(row: 0, section: self.selectedIndex+sumAmount)) as? BlogCell
    ...
    func comment_Count() {
        guard let cell = self.tableView.cellForRow(at: IndexPath(row: 0 , section: self.selectedIndex+sumAmount)) as? BlogCell else { return }
...
//                                BoostPostManager.sharedInstance.boostPosts(targetController: self.targetController, postId: post_id ?? "")
    ...
    static let sharedInstance = GetPostBlog()
    private init() {
