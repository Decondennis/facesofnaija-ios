...
class AddPostVC: UIViewController {
    ...
    @IBOutlet var footerView: DesignView!
    ...
    override func viewDidLoad() {
        ...
        if (self.isFrom_Edit == "1"){
            self.navigationItem.title = NSLocalizedString("Edit Post", comment: "Edit Post")
...
//        controller.blurEffect = .init(style: .regular)
...
//        print("settings adata = \(AppInstance.instance.siteSettings?.config?.postColors)")
    ...
    private func edit_Post(text: String){
        switch status {
        ...
        case .online(.wwan), .online(.wiFi):
            ZKProgressHUD.show()
            EditPostManager.sharedInstance.editPost(text: text, post_id: self.post_id, privacy: self.postPrivacy ?? 0) { (success, authError, error) in
                if (success != nil){
                    ZKProgressHUD.dismiss()
                    self.view.makeToast(success?.action)
                    self.delegate?.editPost(newtext: text, postPrivacy: "\(self.postPrivacy ?? 0)")
...
extension AddPostVC:didSelectUserDelegate,getAddressDelegate{
    func getAddress(address: String) {
        self.location = address
        let cell = tableView.cellForRow(at: IndexPath(row: 0, section: 1)) as! AddPostSectionTwoTableItem
        let attrs1 = [NSAttributedString.Key.foregroundColor : UIColor.black]
        let attrs2 = [NSAttributedString.Key.foregroundColor : UIColor.hexStringToUIColor(hex: ControlSettings.appMainColor)]
        let attributedString1 = NSMutableAttributedString(string: "  At ", attributes:attrs1)
        let attributedString2 = NSMutableAttributedString(string: address, attributes:attrs2)
        attributedString1.append(attributedString2)
//        cell.addressLabel.text  = "\("  At ")\(address)"
//        cell.addressLabel.numberOfLines = 2
        cell.addressLabel.attributedText = attributedString1
        cell.addressBtn.isUserInteractionEnabled = true
        print(address)
    }
