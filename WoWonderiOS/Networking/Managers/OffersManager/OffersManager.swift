...
//             APIClient.Params.offset:offset,
        ...
        AF.request(APIClient.Offers.getOffersApi + access_token, method: .post, parameters: params, encoding: URLEncoding.default, headers: nil).responseJSON { (response) in
            if response.value != nil{
                guard let res = response.value as? [String:Any] else {return}
                guard let apiStatusCode = res["api_status"] as? Any else {return}
                if apiStatusCode as? Int == 200{
                    
                    let result = GetOffersModel.GetOffersSuccessModel.init(json: res)
