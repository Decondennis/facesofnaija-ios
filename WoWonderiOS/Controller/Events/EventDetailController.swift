...
class EventDetailController: UIViewController,GMSMapViewDelegate,CLLocationManagerDelegate,UIScrollViewDelegate,editPostDelegate{
    ...
    override func viewDidLoad() {
        ...
        self.goBtn.borderColor = UIColor.hexStringToUIColor(hex: ControlSettings.buttonColor)
        self.goBtn.setTitleColor(UIColor.hexStringToUIColor(hex: ControlSettings.buttonColor), for: .normal)
        self.interestedBtn.setTitleColor(UIColor.hexStringToUIColor(hex: ControlSettings.buttonColor), for: .normal)
        self.interestedBtn.borderColor = UIColor.hexStringToUIColor(hex: ControlSettings.buttonColor)
        self.moreBtn.borderColor = UIColor.hexStringToUIColor(hex: ControlSettings.buttonColor)
        self.moreBtn.setTitleColor(UIColor.hexStringToUIColor(hex: ControlSettings.buttonColor), for: .normal)
...
//        self.navigationController?.interactivePopGestureRecognizer?.isEnabled = false
      ...
      NotificationCenter.default.addObserver(self,selector:#selector(self.EditEvent(_:)),name: NSNotification.Name("EditEvent"),object: nil)
        ...
        self.startLabel.textColor = UIColor.hexStringToUIColor(hex: ControlSettings.appMainColor)
        self.endLabel.textColor = UIColor.hexStringToUIColor(hex: ControlSettings.appMainColor)
...
NotificationCenter.default.removeObserver(self, name: NSNotification.Name(rawValue: "Notifire"), object: nil)
    ...
    func editPost(newtext: String, postPrivacy: String) {
        self.eventPosts[self.selectedIndex]["postText"] = newtext
        self.eventPosts[self.selectedIndex]["postPrivacy"] = postPrivacy
    ...
    private func loadData(){
        ...
        if let isGoing = self.eventDetail["is_going"] as? Bool {
            if isGoing == false {
                ...
                self.goBtn.backgroundColor = UIColor.hexStringToUIColor(hex: ControlSettings.buttonColor)
//                    UIColor.hexStringToUIColor(hex: "984243")
        ...
        if let isInterested = self.eventDetail["is_interested"] as? Bool{
            if isInterested == false {
                ...
                self.interestedBtn.backgroundColor = UIColor.hexStringToUIColor(hex: ControlSettings.buttonColor)
//                    UIColor.hexStringToUIColor(hex: "984243")
