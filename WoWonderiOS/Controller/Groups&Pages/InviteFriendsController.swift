...
class InviteFriendsController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        if self.pageId != "" {
           ...
           self.getNotCommunityMembers(communityId: Int(self.communityId) ?? 10)
        }
        else {
    ...
    private func getNotCommunityMembers (communityId : Int) {
        
        MemberNot_inGroupManager.sharedInstance.getNotGroupMember(groupId: communityId, completionBlock: { (success, authError, error) in
    ...
    private func AddMembertoCommunity(user_id : String){
        AddMembertoGroupManager.sharedInstance.addMembertoGroup(groupId: self.groupId, userId: user_id) { (success, authError, error) in
            if success != nil {
                print(success?.message)
            }
    ...
    @IBAction func addMember (sender:UIButton) {
        ...
        if self.communityId != "" {
        self.AddMembertoCommunity(user_id: userId)
        }
        else {
