...
class MyGroupsandMyPagesController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        if self.type == "group"{
            self.shareLabel.text = NSLocalizedString("Share to a Group", comment: "Share to a Group")
            self.getGroups()
        }
        else {
            self.shareLabel.text = NSLocalizedString("Share to a Page", comment: "Share to a Page")
            self.getPages()
        }
        
        self.closeBtn.setTitleColor(UIColor.hexStringToUIColor(hex: ControlSettings.buttonColor), for: .normal)
        self.createBtn.setTitleColor(UIColor.hexStringToUIColor(hex: ControlSettings.buttonColor), for: .normal)
