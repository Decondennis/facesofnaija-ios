...
class GetPostShare: AddReactionDelegate,comment_CountsDelegate {
    ...
    func  getsharePost (targetController : UIViewController ,tableView : UITableView, indexpath:IndexPath, postFile : String, array : [[String:Any]]) -> UITableViewCell {
        
        AppInstance.instance.index = indexpath.row
        print("shareIndex",AppInstance.instance.index)
...
//        let index = array[indexpath.row]
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: "PostShare") as! PostShareCell
        cell.targetController = targetController
        ...
        if let time = index["post_time"] as? String{
            cell.sharedTimeLabel.text! = time
        }
        if let textStatus = index["postText"] as? String {
            cell.sharedPostText.text! = textStatus.htmlToString
        }
        ...
        if let publisher = index["publisher"] as? [String:Any] {
            ...
            if let avatarUrl =  publisher["avatar"] as? String {
                ...
                cell.sharedProfileImage.kf.setImage(with: url)
            }
        ...
        if let sharedInfo = index["shared_info"] as? [String:Any]{
            cell.sharePostArray.append(sharedInfo)
            print(sharedInfo)
        }
        
        if let group_recipient = index["group_recipient"] as? [String:Any]{
            if let group_name = group_recipient["group_title"] as? String{
        ...
        cell.sharedPostText.handleURLTap { (URL) in
            print("Tap URL")
            UIApplication.shared.open(URL, options: [:], completionHandler: nil)
        }
        cell.sharedPostText.handleHashtagTap { (hash) in
            print(hash)
        ...
        imageAttachment1.image = UIImage(named:"veirfied")
        imageAttachment2.image = UIImage(named: "flash-1")
        let imageOffsetY: CGFloat = -2.0
        imageAttachment1.bounds = CGRect(x: 0, y: imageOffsetY, width: 15.0, height: 15.0)
        imageAttachment2.bounds = CGRect(x: 0, y: imageOffsetY, width: 11.0, height: 14.0)
        ...
        if groupName != ""{
            let attributedString1 = NSMutableAttributedString(string: profileName, attributes:attrs1)
            let attributedString2 = NSMutableAttributedString(string: " ", attributes:attrs2)
            let attributedString3 = NSMutableAttributedString(attributedString: attechmentString1)
            let attributedString4 = NSMutableAttributedString(string: " ", attributes:attrs2)
            let attributedString5 = NSMutableAttributedString(attributedString: attechmentString2)
            ...
            let attributedString6 = NSMutableAttributedString(string:  " shared a post ", attributes:attrs1)
            let attributedString7 = NSMutableAttributedString(attributedString: attechmentString)
            let attributedString8 = NSMutableAttributedString(string: "\(" ")\(groupName)", attributes:attrs1)
            ...
            cell.shareUserName.attributedText = attributedString1
        }
            ...
            let attributedString7 = NSMutableAttributedString(string: "shared a post", attributes:attrs1)
        ...
        cell.sharedMoreBtn.tag = indexpath.row
        cell.shareUserName.tag = indexpath.row
        cell.sharedProfileImage.tag = indexpath.row
        ...
        cell.sharedProfileImage.addGestureRecognizer(gesture)
        cell.sharedProfileImage.isUserInteractionEnabled = true
        cell.shareUserName.isUserInteractionEnabled = true
        cell.shareUserName.addGestureRecognizer(gestureonLabel)
        ...
        cell.sharedMoreBtn.addTarget(self, action: #selector(self.More(sender:)), for: .touchUpInside)
    ...
    @IBAction func NormalTapped(gesture: UIGestureRecognizer){
        ...
        case .unknown, .offline:
            ...
            guard let cell = self.tableView.cellForRow(at: IndexPath(row: 0, section: view.tag + sumAmount)) as? PostShareCell else { return }
            ...
            if let reactions = self.postArray[gesture.view?.tag ?? 0]["reaction"] as? [String:Any]{
                ...
                if let is_react = reactions["is_reacted"] as? Bool{
                    if is_react == true{
                        ...
                        var localPostArray = (self.postArray[gesture.view?.tag ?? 0]["reaction"] as? [String:Any]) ?? [String:Any]()
    ...
    func addReaction(reation: String) {
        let cell = self.tableView.cellForRow(at: IndexPath(row: 0, section: self.selectedIndex+sumAmount)) as? PostShareCell
    ...
    func comment_Count() {
        guard let cell = self.tableView.cellForRow(at: IndexPath(row: 0, section: self.selectedIndex+sumAmount)) as? PostShareCell else { return }
            
            let count = Int(self.comment_count) ?? 0
            let total = count + 1
            cell.commentsCountBtn.setTitle("\(" ")\(total) Comments", for: .normal)
...
//                            BoostPostManager.sharedInstance.boostPosts(targetController: self.targetController, postId: post_id ?? "")
    ...
    static let sharedInstance = GetPostShare()
    private init() {    NotificationCenter.default.addObserver(self, selector: #selector(self.networkStatusChanged(_:)), name: Notification.Name(rawValue: ReachabilityStatusChangedNotification), object: nil)
