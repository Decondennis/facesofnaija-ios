        ...
        performUIUpdatesOnMain {
            self.savePost(action: action, postId: postId) { (success, authError, error) in
                 if success != nil{
                    ...
                    let actions = success?.action ?? ""
                    if (actions == "saved post"){
                        targetController.view.makeToast(NSLocalizedString("The post successfully saved", comment: "The post successfully saved"))
                    }
                    else{
                        targetController.view.makeToast(NSLocalizedString("Post UnSaved", comment: "Post UnSaved"))
                    }
