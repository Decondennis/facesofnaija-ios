...
class AuthenticationManager {
    ...
    func loginAuthentication (userName : String, password : String,deviceId:String, completionBlock : @escaping (_ Success:AuthenticationModel.LoginAuth_SuccessModel?, _ AuthError : AuthenticationModel.LoginAuth_ErrorModel?, Error?)->()) {
        ...
        AF.request(APIClient.Login_Authentication.loginAuthApi, method: .post, parameters: params, encoding: URLEncoding.default, headers: nil).responseJSON { (response) in
    ...
    func signUPAuthentication(userName : String, password : String, email : String,confirmPassword : String ,deviceId:String,gender:String = "male",completionBlock : @escaping (_ Success:AuthenticationModel.SignUp_SuccessModel?, _ AuthError : AuthenticationModel.SignUp_ErrorModel?, Error?)->()) {
        
        var params: [String:Any] = [APIClient.Params.serverKey : APIClient.SERVER_KEY.Server_Key, APIClient.Params.userName : userName, APIClient.Params.password : password, APIClient.Params.email : email, APIClient.Params.confirmPassword : confirmPassword,APIClient.Params.ios_n_device_id : deviceId]
        if !gender.isEmpty {
            params["gender"] = gender
        }
        print("API = \(APIClient.SignUp_Authentication.signupAuthApi)")
        AF.request(APIClient.SignUp_Authentication.signupAuthApi, method: .post, parameters: params, encoding: URLEncoding.default, headers: nil).responseJSON { (response) in
            if response.value != nil {
                guard let res = response.value as? [String:Any] else {return}
                guard let apiStatusCode = res["api_status"]  as? Any else {return}
                let apiCode = (apiStatusCode as? Int) ?? (Int("\(apiStatusCode)"))
                if apiCode == 200 {
                    guard let allData = try? JSONSerialization.data(withJSONObject: response.value, options: [])else {return}
                    ...
                    let result = AuthenticationModel.SignUp_SuccessModel(apiStatus: 220, accessToken: "", userID: (res["user_id"] as? String) ?? "")
                    completionBlock(result,nil,nil)
                }
                else {
                    guard let allData = try? JSONSerialization.data(withJSONObject: response.value, options: [])else {return}
