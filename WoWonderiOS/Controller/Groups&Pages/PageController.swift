...
class PageController: UIViewController,EditPageDelegete,DeletePageDelegate,PageMoreDelegate,PageRatingDelegate,editPostDelegate{
...
//        interstitial.load(GADRequest())
    ...
    func editPost(newtext: String, postPrivacy: String) {
        self.pagePostArray[self.selectedIndex]["postText"] = newtext
        self.pagePostArray[self.selectedIndex]["postPrivacy"] = postPrivacy
        self.tableView.reloadData()
    }
                ...
                if action_type == "0"{
                    if let is_liked = self.page_data["is_liked"] as? Bool{
                        if is_liked == false{
                            ...
                            cell.callActionBtn.backgroundColor = UIColor.hexStringToUIColor(hex: ControlSettings.buttonColor)
//                                UIColor.hexStringToUIColor(hex: "#984243")
                            self.likePage()
