...
class GetPostWithLink : AddReactionDelegate,SharePostDelegate,comment_CountsDelegate{

    
    static let sharedInstance = GetPostWithLink()
    private init() {
    ...
    func getPostLink(targetController :UIViewController,tableView : UITableView, indexpath:IndexPath, postLink : String, array : [[String:Any]],stackViewHeight: CGFloat,viewHeight: CGFloat, isHidden: Bool, viewColor :UIColor) -> UITableViewCell {
...
//        cell.stackViewHeight.constant = stackViewHeight
        ...
        tableView.estimatedRowHeight = 490.0
        var names = ""
        var isPro = ""
        var isVerified = ""
...
//                cell.profileName.text! = profilename
        ...
        if let link = index["postLink"] as? String{
            cell.linkLabel.text = link.htmlToString
        }
        ...
        if let postLinkImage = index["postLinkImage"] as? String {
            if postLinkImage == ""{
                cell.heightConstaint.constant = 35.0
            }
            else{
                cell.heightConstaint.constant = 180.0
            let url = URL(string: postLinkImage)
//            cell.linkImage.kf.setImage(with: url, placeholder: nil, options: [], progressBlock: nil) { [weak self](image, error, cache, url) in
//                if image != nil {
//                    cell.linkImage.layoutIfNeeded()
//                    cell.linkImage?.clipsToBounds = true
//                    
//                }
//            }
        }
        ...
        if let postLinkTitle = index["postLinkTitle"] as? String{
            cell.postLinkTitle.text! = postLinkTitle.htmlToString
        }
        
        if let postLinkContent = index["postLinkContent"] as? String {
            cell.postLinkContent.text! = postLinkContent.htmlToString
        }
        ...
        cell.postLinkContent.handleURLTap { (URL) in
            print("Tap URL")
            UIApplication.shared.open(URL, options: [:], completionHandler: nil)
        }
        cell.postLinkContent.handleHashtagTap { (hash) in
            print(hash)
        ...
        cell.postTitle.handleURLTap { (URL) in
            print("Tap URL")
            UIApplication.shared.open(URL, options: [:], completionHandler: nil)
        }
        cell.postTitle.handleHashtagTap { (hash) in
            print(hash)
            let Storyboard = UIStoryboard(name: "Search", bundle: nil)
            let vc = Storyboard.instantiateViewController(withIdentifier: "PostHashTagVC") as! PostHashTagController
            vc.hashtag = hash
    ...
    @IBAction func NormalTapped(gesture: UIGestureRecognizer){
        ...
        case .online(.wwan), .online(.wiFi):
            ...
            guard let cell = self.tableView.cellForRow(at: IndexPath(row: 0, section: gesture.view!.tag + sumAmount)) as? PostWithLinkCell else { return }
    ...
    func addReaction(reation: String) {
        ...
        let cell = self.tableView.cellForRow(at: IndexPath(row: 0, section: self.selectedIndex+sumAmount)) as? PostWithLinkCell
    ...
    func comment_Count() {
        guard let cell = self.tableView.cellForRow(at: IndexPath(row: 0, section: self.selectedIndex+sumAmount)) as? PostWithLinkCell else { return }
