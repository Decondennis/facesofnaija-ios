...
class GetPostSticker :AddReactionDelegate,SharePostDelegate,comment_CountsDelegate{
    ...
    func getPostSticker (targetController :UIViewController,tableView : UITableView, indexpath:IndexPath, postFile : String, array : [[String:Any]],stackViewHeight: CGFloat,viewHeight: CGFloat, isHidden: Bool, viewColor :UIColor) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: "GifCell") as! GifImageCell
        ...
        cell.commentCountBtn.isHidden = isHidden
        ...
        if let postSticker = index["postSticker"] as? String{
            let url = URL(string: postSticker)
                        cell.gifImageView.sd_setImage(with: url, placeholderImage: nil, options: []) { (image, error, cache, url) in
                if image != nil {
                    cell.heightContraint.constant = GetPosts.getAspectRatioAccordingToiPhones(cellImageFrame: cellFrame,downloadedImage: image!)
                }
                    
                else {
                    cell.heightContraint.constant = 220.0
                }
            }
        }
        ...
        if let commentsCount = index["post_comments"] as? String{
            cell.commentCountBtn.setTitle("\(" ")\(commentsCount)", for: .normal)
        }
        
        for i in self.commentsIndex{
            if i["index"] as? Int == indexpath.row{
                cell.commentCountBtn.setTitle("\(" ")\(i["count"] ?? "")", for: .normal)
            }
        }
        ...
        cell.commentCountBtn.addTarget(self, action: #selector(self.GotoComments(sender:)), for: .touchUpInside)
    ...
    func comment_Count() {
        let cell = self.tableView.cellForRow(at: IndexPath(row: 0, section: self.selectedIndex+sumAmount)) as! GifImageCell
            ...
            let total = count + 1
            cell.commentCountBtn.setTitle("\(" ")\(total)", for: .normal)
...
//                            BoostPostManager.sharedInstance.boostPosts(targetController: self.targetController, postId: post_id ?? "")
    ...
    static let sharedInstance = GetPostSticker()
    private init() {
