...
class CommunityByCategoryController: UIViewController {
    ...
    private func getSuggestedGroups(){
        ...
        case .unknown, .offline:
            ...
            self.view.makeToast("Internet Connection Failed")
        case .online(.wwan),.online(.wiFi):
            GetSuggestedGroupManager.sharedInstance.getGroups(type: "groups", limit: 30) { (success, authError, error) in
                if (success != nil){
                    for i in success!.data{
                        self.groups.append(i)
                    }
...
extension CommunityByCategoryController: UITableViewDataSource,UITableViewDelegate,JoinGroupDelegate,DeleteGroupDelegate{
