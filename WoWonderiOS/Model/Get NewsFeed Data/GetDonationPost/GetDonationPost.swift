...
class GetDonationPost: AddReactionDelegate,SharePostDelegate,comment_CountsDelegate{
    ...
    var pro_name: String? = nil
    
    let playRing = URL(fileURLWithPath: Bundle.main.path(forResource: "button", ofType: "mp3")!)
    var audioPlayer = AVAudioPlayer()
    
    func getDonationpost(targetController : UIViewController ,tableView : UITableView, indexpath:IndexPath, array : [[String:Any]],stackViewHeight: CGFloat,viewHeight: CGFloat, isHidden: Bool, viewColor :UIColor) -> UITableViewCell {
        self.targetController = targetController
        self.tableView = tableView
        self.postArray = array
        let index = postArray[indexpath.row]
        let cell = tableView.dequeueReusableCell(withIdentifier: "DonationPost") as! DonationPostCell
        tableView.rowHeight = 430.0
        var names = ""
        var isPro = ""
        var isVerified = ""
        ...
        if let time = index["post_time"] as? String{
            ...
            cell.donationPostTime.text = time
        }
...
//                cell.profileName.text! = profilename
                names = profilename
//                cell.donationUsername.text! = profilename
            }
        ...
        if let donationPost = index["fund_data"] as? [String:Any]{
            if let image = donationPost["image"] as? String{
                let url = URL(string: image)
                cell.donationimage.kf.setImage(with: url)
            }
            if let title = donationPost["title"] as? String{
                cell.donationTitle.text = title
            }
            if let descrip = donationPost["description"] as? String{
                cell.donationDescription.text = descrip
            }
            
            if let amountRaised = donationPost["raised"] as? Int{
                cell.amountRaised.text = "\("$")\(amountRaised)\(".00")"
            }
            if let amount = donationPost["amount"] as? String{
                cell.amount.text = "\("$")\(amount)\(".00")"
            }
            if let bar = donationPost["bar"] as? Int{
                cell.progressView.progress = Float(bar)/100
            }
        }
        cell.btnAction = {[unowned self] in
        ...
        cell.moreBtn.addTarget(self, action: #selector(self.More(sender:)), for: .touchUpInside)
          ...
          let attributedString6 = NSMutableAttributedString(string: "\(" ")\("Created a donation request")", attributes:attrs3)
    ...
    @IBAction func NormalTapped(gesture: UIGestureRecognizer){
        ...
        case .online(.wwan), .online(.wiFi):
            guard let cell = self.tableView.cellForRow(at: IndexPath(row: 0, section: gesture.view!.tag + sumAmount)) as? DonationPostCell else { return }
    ...
    func addReaction(reation: String) {
        let cell = self.tableView.cellForRow(at: IndexPath(row: 0, section: self.selectedIndex+sumAmount)) as? DonationPostCell
    ...
    func comment_Count() {
        guard let cell = self.tableView.cellForRow(at: IndexPath(row: 0, section: self.selectedIndex+sumAmount)) as? DonationPostCell else { return }
...
//                            BoostPostManager.sharedInstance.boostPosts(targetController: self.targetController, postId: post_id ?? "")
   ...
   static let sharedInstance = GetDonationPost()
   private init() {}
}
